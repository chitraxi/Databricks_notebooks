...
df = spark.sql("""select p.auth_type,p.method_advanced, p.gateway, response.id,response.gateway,action,raw_response, response.source_id, response_body,p.created_date 
    from (
    ...
    left join (
        ...
        and source_type = 'payment' and action in ('pay_verify','verify','pay_init') AND (case when gateway = 'billdesk_optimizer' and action = 'pay_verify' then status != 0 else true end ) order by id) as response on response.source_id = p.id
                """)
