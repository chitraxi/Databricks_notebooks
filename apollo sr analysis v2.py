...
SR_filtered = SR[(SR["Delta_SR_agg"] < -1) & (SR["total_agg"] > 3000)]
...
merged_df = drilled_SR.merge(SR_filtered,
...
top5_vmm_sr = merged_df.groupby(["method_advanced", "aggregator"]).apply(lambda x: x.nsmallest(5, "VMM_SR")).reset_index(drop=True)
...
top5_vmm_tx = merged_df.groupby(["method_advanced", "aggregator"]).apply(lambda x: x.nsmallest(5, "VMM_TX")).reset_index(drop=True)
...
for index, row in SR_filtered.iterrows():
    ...
    filtered_top5_vmm_SR_list = top5_vmm_sr[
        ...
        (top5_vmm_sr[("aggregator", "")] == aggregator)
    ].iterrows()
    ...
    for inner_index, inner_row in filtered_top5_vmm_SR_list:
        ...
        (summary_error_df[("aggregator", "")] == aggregator) & (summary_error_df[("merchant_id", "")] == merchant_id) 
    ...
    filtered_top5_vmm_tx_list = top5_vmm_tx[
        ...
        (top5_vmm_tx[("aggregator", "")] == aggregator)
    ].iterrows()
