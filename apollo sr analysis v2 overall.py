...
end_date = '2024-07-30'
previous_start_date = '2024-06-01'
previous_end_date = '2024-06-30'
...
df_base_data = spark.sql(f"""
    ...
    FROM 
        ...
        where a.id not in (select id from realtime_hudi_api.payments where created_date between '{previous_start_date}' AND '{end_date}' and cps_route = 100 and  concat(method,created_date)   IN ('upi2024-07-19', 'upi2024-07-20'))
...
top5_vmm_sr = merged_df.groupby(["method_advanced"]).apply(lambda x: x.nsmallest(5, "VMM_SR")).reset_index(drop=True)
...
top5_vmm_tx = merged_df.groupby(["method_advanced"]).apply(lambda x: x.nsmallest(5, "VMM_TX")).reset_index(drop=True)
