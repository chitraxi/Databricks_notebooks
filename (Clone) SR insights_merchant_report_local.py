...
end_date = '2024-09-30'
previous_start_date = '2024-08-01'
previous_end_date = '2024-08-31'
...
agg_merged_df[(agg_merged_df["merchant_id"] == "IarpS61cvRhDtA") & (agg_merged_df["method_advanced"] == "UPI Intent")]
...
bottom2_vmm_sr = agg_merged_df.groupby(["merchant_id"]).apply(lambda x: x.nsmallest(2, "VMM_SR")).reset_index(drop=True)
top2_vmm_sr = agg_merged_df.groupby(["merchant_id"]).apply(lambda x: x.nlargest(2, "VMM_SR")).reset_index(drop=True)

# COMMAND ----------

bottom2_vmm_sr[bottom2_vmm_sr["merchant_id"] == "ENiVVm6cLOMkaa"]
...
bottom2_vmm_tx = agg_merged_df.groupby(["merchant_id"]).apply(lambda x: x.nsmallest(2, "VMM_TX")).reset_index(drop=True)
top2_vmm_tx = agg_merged_df.groupby(["merchant_id"]).apply(lambda x: x.nlargest(2, "VMM_TX")).reset_index(drop=True)
...
error_df = df_failed.pivot_table(
    index=["method_advanced", "aggregator", "merchant_id", "name", "network_tokenised_payment","internal_error_code"],
    columns="period",
    values=["payment_attempts", "payment_success"],
    aggfunc="sum"
).fillna(0).reset_index()
...
error_df = error_df.merge(aggregated_SR_df,on=['merchant_id','aggregator','method_advanced','network_tokenised_payment'],how='left')

# COMMAND ----------

## Error % in T-1
error_df['Error_per_T-1'] = round(error_df['payment_attempts_x','T-1']*100/error_df['payment_attempts_y','T-1'],2)

# COMMAND ----------

## Error % in T-2
error_df['Error_per_T-2'] = round(error_df['payment_attempts_x','T-2']*100/error_df['payment_attempts_y','T-2'],2)

# COMMAND ----------

## Change in Error %
##error_df['Error_per_delta'] = error_df['Error_per_T-1'] - error_df['Error_per_T-2']

# Step 1: Confirm column existence
if (("Error_per_T-1") in error_df.columns) and (("Error_per_T-2") in error_df.columns):
    # Step 2: Define the filter condition
    condition = error_df[("Error_per_T-2")] > 0
...
error_df[error_df["merchant_id"] == "ENiVVm6cLOMkaa"]

# COMMAND ----------

# Group by the specified columns and sort within each group by payment_attempts (For Top 2 VMM SR)
sorted_error_df = error_df.groupby(["method_advanced", "aggregator", "merchant_id", "name_x", "network_tokenised_payment"], as_index=False).apply(lambda x: x.sort_values(by=("Error_per_delta"), ascending=True))
...
# Group by the specified columns and sort within each group by payment_attempts (For bottom 2 VMM SR)
sorted_error_df_bottom_vmm_sr = error_df.groupby(["method_advanced", "aggregator", "merchant_id", "name_x", "network_tokenised_payment"], as_index=False).apply(lambda x: x.sort_values(by=("Error_per_delta"), ascending=False))

# Reset the index
sorted_error_df_bottom_vmm_sr = sorted_error_df_bottom_vmm_sr.reset_index(drop=True)

# Show the sorted DataFrame
print(sorted_error_df_bottom_vmm_sr)

# COMMAND ----------

sorted_error_df_bottom_vmm_sr[(sorted_error_df_bottom_vmm_sr["merchant_id"] == "ENiVVm6cLOMkaa") & (sorted_error_df_bottom_vmm_sr["aggregator"] == "cashfree")]

# COMMAND ----------

sorted_error_df.head()

# COMMAND ----------

# Group by specified columns and get top three rows for each group (FOR TOP 2 VMM SR)
...
limited_error_df = sorted_error_df.groupby(["method_advanced", "aggregator", "merchant_id", "name_x", "network_tokenised_payment"]).head(3)
# Show the limited DataFrame
limited_error_df

# COMMAND ----------

# Group by specified columns and get top three rows for each group (FOR BOTTOM 2 VMM SR)
#limited_error_df = sorted_error_df.groupby(["method_advanced", "aggregator", "merchant_id", "name", "network", "network_tokenised_payment", "bank"]).head(3)

limited_error_df_bottom_vmm_sr = sorted_error_df_bottom_vmm_sr.groupby(["method_advanced", "aggregator", "merchant_id", "name_x", "network_tokenised_payment"]).head(3)
# Show the limited DataFrame
limited_error_df_bottom_vmm_sr

# COMMAND ----------

limited_error_df_bottom_vmm_sr[(limited_error_df_bottom_vmm_sr["merchant_id"] == "ENiVVm6cLOMkaa") & (limited_error_df_bottom_vmm_sr["aggregator"]== "cashfree")]

# COMMAND ----------

limited_error_df[limited_error_df["merchant_id"] == "ENiVVm6cLOMkaa"]

# COMMAND ----------

# Perform inner join on all common columns
top2_vmmSR_error_df = top2_vmm_sr.merge(limited_error_df, left_on=["method_advanced", "aggregator", "merchant_id", "name", "network_tokenised_payment"],right_on = ["method_advanced", "aggregator", "merchant_id", "name_x", "network_tokenised_payment"], how="inner")
...
# Perform inner join on all common columns
bottom2_vmmSR_error_df = bottom2_vmm_sr.merge(limited_error_df_bottom_vmm_sr, left_on=["method_advanced", "aggregator", "merchant_id", "name", "network_tokenised_payment"],right_on = ["method_advanced", "aggregator", "merchant_id", "name_x", "network_tokenised_payment"], how="inner")

# COMMAND ----------

top2_vmmSR_error_df[top2_vmmSR_error_df["merchant_id"] == "ENiVVm6cLOMkaa"]
...
top2_vmmSR_error_df
summary_error_df = top2_vmmSR_error_df[top2_vmmSR_error_df[("Error_per_delta", "")] < 0]

summary_error_df

# COMMAND ----------

summary_error_df_bottom_vmm_sr = bottom2_vmmSR_error_df[bottom2_vmmSR_error_df[("Error_per_delta", "")] > 0]

summary_error_df_bottom_vmm_sr

# COMMAND ----------

summary_error_df[summary_error_df["merchant_id"] == "ENiVVm6cLOMkaa"]
...
summary_error_df[
        (summary_error_df[("method_advanced", "")] == "Credit Card") &
        (summary_error_df[("aggregator", "")] == "cashfree") & (summary_error_df[("merchant_id", "")] == "ENiVVm6cLOMkaa") 
        & (summary_error_df[("network_tokenised_payment", "")] == "1") 
        ]

# COMMAND ----------

merged_df[
        (merged_df[("method_advanced", "")] == "Credit Card") &
        (merged_df[("aggregator", "")] == "cashfree") & (merged_df[("merchant_id", "")] == "ENiVVm6cLOMkaa") 
        & (merged_df[("network_tokenised_payment", "")] == "1") 
        ].sort_values(by=["VMM_SR"], ascending=True).head(3).reset_index()

# COMMAND ----------

print("overall sr\033[1m\033[38;5;196m hai ji")

# COMMAND ----------

check = summary_error_df_bottom_vmm_sr[(summary_error_df_bottom_vmm_sr['merchant_id']=='ENiVVm6cLOMkaa') & (summary_error_df_bottom_vmm_sr['aggregator']=='cashfree')]

# COMMAND ----------

summary_error_df_bottom_vmm_sr[(summary_error_df_bottom_vmm_sr['merchant_id']=='ENiVVm6cLOMkaa') & (summary_error_df_bottom_vmm_sr['aggregator']=='cashfree')]

# COMMAND ----------

check['type'] = check['network_tokenised_payment']==str(1)
...
for index,rows in check:
    error = rows[("internal_error_code", "")]
    delta_error = round(rows["Error_per_delta",""],2)
    current_attempts = round(rows["Error_per_T-1",""],2)
    previous_attempts = round(rows["Error_per_T-2",""],2)
    print(f"        {error} - increased by {delta_error}%. Previous error count - {previous_attempts}% , Current error count- {current_attempts}%")
...
for index, row in overall_agg_sr.iterrows():
    ...
    filtered_bottom2_vmm_SR_list = bottom2_vmm_sr[
        (bottom2_vmm_sr[("merchant_id", "")] == merchant_id) ].iterrows()
    ...
    print("#" * 20 + "\n")  # Medium heading
    Counter1 = 1
    print(filtered_bottom2_vmm_SR_list)
    for inner_index, inner_row in filtered_bottom2_vmm_SR_list:
        ...
        if network_tokenised_payment==str(1):
            tokenization_type = "tokenized payments"
        elif network_tokenised_payment != str(1):
        ...
        #netwrok_level_SR_dip = netwrok_level_SR_dip[netwrok_level_SR_dip["VMM_SR"]<= -0.05]
        ...
        if inner_method in ("Credit Card", "Debit Card", "CC emi", "DC emi"):
            print(f"    {Counter1}. On {inner_method} - {inner_aggregator} (for {tokenization_type})\n             - SR dropped by {vmm_sr}%")
            insights_error_trend_neg += f"    \t{Counter1}. On {inner_method} - {inner_aggregator} (for {tokenization_type})\n             - SR dropped by {vmm_sr}%\n             - Networks impacted : {top_networks}\n"

        if inner_method in ("UPI Intent", "UPI Collect"):
            print(f"    {Counter1}. On {inner_method} - {inner_aggregator}, SR dropped by {vmm_sr}%")
            insights_error_trend_neg += f"    \t{Counter1}. For {inner_method} - {inner_aggregator}\n             - SR dropped by {vmm_sr}%\n"
        ...
        filtered_errors = summary_error_df_bottom_vmm_sr[
        (summary_error_df_bottom_vmm_sr[("method_advanced", "")] == inner_method) &
        (summary_error_df_bottom_vmm_sr[("aggregator", "")] == inner_aggregator) & (summary_error_df_bottom_vmm_sr[("merchant_id", "")] == merchant_id) 
        & (summary_error_df_bottom_vmm_sr[("network_tokenised_payment", "")] == network_tokenised_payment) 
        ].iterrows()
        
        
        for index,error_row in filtered_errors:
            error = error_row[("internal_error_code", "")]
            delta_error = round(error_row["Error_per_delta",""],2)
            current_attempts = round(error_row["Error_per_T-1",""],2)
            previous_attempts = round(error_row["Error_per_T-2",""],2)
            print(f"        {error} - increased by {delta_error}%. Previous error% - {previous_attempts}% , Current error% - {current_attempts}%")
            insights_error_trend_neg += f"       \t \t   - {error} increased by {delta_error}%. (Previous error% - {previous_attempts}% , Current error% - {current_attempts}%) \n"
        Counter1 +=1

    print("#" * 20)  # Medium heading
    print("SR drop due to change in Volume:")
    print("#" * 20 + "\n")  # Medium heading
    filtered_bottom2_vmm_tx_list = bottom2_vmm_tx[
        (bottom2_vmm_tx[("merchant_id", "")] == merchant_id) ].iterrows()

    print(filtered_bottom2_vmm_tx_list)
    Counter2 = 1 

    for inner_index, inner_row in filtered_bottom2_vmm_tx_list:
        # Extract information for the summary
        inner_method = inner_row[("method_advanced", "")]
        inner_aggregator = inner_row[("aggregator", "")]
        merchant_id = inner_row[("merchant_id", "")]
        ...
        if inner_method in ("Credit Card", "Debit Card", "CC emi", "DC emi"):
            print(f"        {Counter2}. On {inner_method} - {inner_aggregator} (for {tokenization_type})\n             - SR dropped by\033[1m\033[38;5;196m {vmm_tx}%\033[0m due to change in volume by\033[1m {delta_traffic}\033[0m.\n                • Previous attempts {previous_attempts} and Current attempts - {current_attempts }.\n                • Network impacted : \033[1m{top_networks}\033[0m")
            insights_volume_change_neg += f"        \t{Counter2}. On {inner_method} - {inner_aggregator} (for {tokenization_type})\n             - SR dropped by {vmm_tx}% due to change in volume by {delta_traffic}%.\n                • Previous attempts {previous_attempts} and Current attempts - {current_attempts }.\n                • Network impacted : {top_networks}\n \n"

        if inner_method in ("UPI Intent", "UPI Collect"):
            print(f"        {Counter2}. On {inner_method} - {inner_aggregator}\n             - SR dropped by\033[1m\033[38;5;196m {vmm_tx}%\033[0m due to change in volume by {delta_traffic}.\n                • Previous attempts {previous_attempts} and Current attempts - {current_attempts }")
            insights_volume_change_neg += f"        \t{Counter2}. On {inner_method} - {inner_aggregator}\n             - SR dropped by {vmm_tx}% due to change in volume by {delta_traffic}%.\n                • Previous attempts {previous_attempts} and Current attempts - {current_attempts }\n \n"

        if inner_method in ("netbanking"):
            print(f"        {Counter2}. For {inner_method} - {inner_aggregator}\n             - SR dropped by\033[1m\033[38;5;196m {vmm_tx}%\033[0m due to change in volume by {delta_traffic}.\n                • Previous attempts {previous_attempts} and Current attempts - {current_attempts }")
            insights_volume_change_neg += f"        \t{Counter2}. On {inner_method} - {inner_aggregator}\n             - SR dropped by {vmm_tx}% due to change in volume by {delta_traffic}%.\n                • Previous attempts {previous_attempts} and Current attempts - {current_attempts}\n \n"
    ...
    print("#" * 20 + "\n")  # Medium heading
    filtered_top2_vmm_SR_list = top2_vmm_sr[
        (top2_vmm_sr[("merchant_id", "")] == merchant_id) ].iterrows()

    counter3 = 1    
    for inner_index, inner_row in filtered_top2_vmm_SR_list:
        # Extract information for the summary
        ...
        if inner_method in ("Credit Card", "Debit Card", "CC emi", "DC emi"):
            print(f"    {counter3}. On {inner_method} - {inner_aggregator} (for {tokenization_type})\n             - SR increased by {vmm_sr}. Networks impacted {top_networks}")
            insights_error_trend_pos += f"    \t{counter3}. On {inner_method} - {inner_aggregator} (for {tokenization_type})\n             - SR increased by {vmm_sr}%.\n             - Networks impacted :  {top_networks}\n"

        if inner_method in ("UPI Intent", "UPI Collect"):
            print(f"    {counter3}. On {inner_method} - {inner_aggregator}\n             - SR increased by {vmm_sr}")
            insights_error_trend_pos += f"    \t{counter3}. On {inner_method} - {inner_aggregator}\n             - SR increased by {vmm_sr}%\n"

        if inner_method in ("netbanking"):
            print(f"    {counter3}. On {inner_method} - {inner_aggregator},SR increased by {vmm_sr}")
            insights_error_trend_pos += f"    \t{counter3}. For {inner_method} - {inner_aggregator}\n             - SR increased by {vmm_sr}%\n"
        ...
        for index,error_row in filtered_errors:
            ...
            current_attempts = round(error_row["Error_per_T-1",""],2) 
            previous_attempts = round(error_row["Error_per_T-2",""],2)
            print(f"        {error} - decreased by {delta_error}%. Previous error count - {previous_attempts}% , Current error count- {current_attempts}%")
            insights_error_trend_pos += f"       \t \t   - {error} decreased by {delta_error}%. (Previous error% - {previous_attempts}% , Current error%- {current_attempts}%) \n"
        counter3 +=1

    print("#" * 20)  # Medium heading
    print("SR incresed due to change in Volume:")
    print("#" * 20 + "\n")  # Medium heading
    filtered_top2_vmm_tx_list = top2_vmm_tx[
        (top2_vmm_tx[("merchant_id", "")] == merchant_id) ].iterrows()

    Counter4 = 1 

    for inner_index, inner_row in filtered_top2_vmm_tx_list:
        # Extract information for the summary
        inner_method = inner_row[("method_advanced", "")]
        inner_aggregator = inner_row[("aggregator", "")]
        merchant_id = inner_row[("merchant_id", "")]
        ...
        if inner_method in ("Credit Card", "Debit Card", "CC emi", "DC emi"):
            print(f"        {Counter4}. On {inner_method} - {inner_aggregator} (for {tokenization_type})\n             - SR increased by\033[1m\033[38;5;70m {vmm_tx}%\033[0m due to change in volume by {delta_traffic}%.\n                • Previous attempts {previous_attempts} and Current attempts: {current_attempts }.\n                • Networks impacted : \033[1m{top_networks}\033[0m")
            insights_volume_change_pos += f"        \t{Counter4}. On {inner_method} - {inner_aggregator}\n             - SR increased by {vmm_tx}% due to change in volume by {delta_traffic}%.\n                • Previous attempts {previous_attempts} and Current attempts: {current_attempts }.\n                • Networks impacted : {top_networks}  \n \n"


        if inner_method in ("UPI Intent", "UPI Collect"):
            print(f"        {Counter4}. On {inner_method} - {inner_aggregator}\n             - SR increased by\033[1m\033[38;5;70m {vmm_tx}%\033[0m due to change in volume by {delta_traffic}%.\n                • Previous attempts {previous_attempts} and Current attempts - {current_attempts }")
            insights_volume_change_pos += f"        \t{Counter4}. On {inner_method} - {inner_aggregator}\n             - SR increased by {vmm_tx}% due to change in volume by {delta_traffic}%.\n                • Previous attempts: {previous_attempts} and Current attempts: {current_attempts } \n \n"
...
##new_df.to_csv("/dbfs/FileStore/Vinayak/Error_check.csv")
